...
def main():
    ...
    parser.add_argument('--switch', metavar='MODE', action='store',
                        help="Set the GPU mode to MODE. You need to log out then log in to apply the change."
                             "Possible modes : nvidia, amd, intel, hybrid-amd, hybrid-intel, ac_auto, auto (auto-detects the mode you may want to switch to).")
    parser.add_argument('--set-startup', metavar='STARTUP_MODE', action='store',
                        help="Set the startup mode to STARTUP_MODE. Possible modes : "
                             "nvidia, amd, intel, hybrid-amd, hybrid-intel, ac_auto")
...
def _get_switch_mode(switch_arg):
    ...
    if switch_arg not in ["auto", "nvidia", "amd", "intel", "hybrid-amd", "hybrid-intel", "ac_auto"]:
    ...
    if switch_arg == "auto":
        ...
        switch_mode = "hybrid-amd" if gpu_mode == "amd" else "amd"
...
def _set_startup_and_exit(startup_arg):
    ...
    if startup_arg not in ["nvidia", "amd", "intel", "hybrid-amd", "hybrid-intel", "ac_auto"]:
