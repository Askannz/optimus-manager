...
By default, the daemon assumes that your display manager service has the name alias `display-manager` (should be the default on Arch, but NOT on Manjaro). If it isn't, you have to specify the name manually so that optimus-manager can find it. See the `login_manager` parameter in the configuration file, in the `[optimus]` section.
...
First, make sure your system is not completely locked up and you can still switch between TTYs with Ctrl+Alt+F1,F2,etc. If you cannot, [refer to this question](STUB).
