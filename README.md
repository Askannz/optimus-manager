...
Currently, if you have Linux installed on an Optimus laptop, there are methods to use your Nvidia GPU :
...
So, an acceptable solution could be to use the second method *on demand* : switching the X session to the Nvidia GPU when you need that extra rendering power, and then switching it back to Intel when you are done, to save battery life.
